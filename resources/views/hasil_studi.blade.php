    ...
    <!-- Modal -->
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-body">
                ...
                <form method='post' action='/simpan_hasil'>
                    <!-- untuk membangun form field pada laravel -->
                    {{ csrf_field() }}
                    <table border='0'>
                        <tr>
                            <td>Mahasiswa&ensp;</td>
                            <td>
                            <select name="nim" id="nim">
                            @foreach($list_mahasiswa as $h)
                                <option value="{{ $h->nim }}">{{ $h->nim }} - {{ $h->nama }}</option>
                            @endforeach
                            </select>
                            </td>
                        </tr>
                        <tr>
                            <td>Nilai&ensp;</td><td><input type='text' name='nilai' required></td>
                        </tr>
                    </table>
...
<div class='container' >
        <div class="table-responsive">
                <table id="example" class="table table-striped" style="font-family: arial, sans-serif; border-collapse: collapse; width: 100%">
                    ...
                    <tbody id="myTable">
                        @foreach($hasil_studi as $h)
                        <tr style="border: 1px solid #dddddd; text-align: left; padding: 8px">
                            <td>{{ $h->nim }}</td>
                            <td>{{ $h->nama }}</td>
                            <td>{{ $h->nilai }}</td>
                            <td align="center"><a href="/hapus_hasil/{{ $h->id }}">Hapus</a></td>
                            <td align="center">
                            <form action='/edit_hasil' method='post'> 
                                {{ csrf_field() }}
